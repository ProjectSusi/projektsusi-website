...
# Optional authentication (not all deployments may have auth enabled)
try:
...
async def query_documents(
    ...
    try:
        ...
        if AUTH_AVAILABLE:
            if current_user:
                logger.info(f"Authenticated query from user: {current_user.username}")
            else:
                logger.info("Unauthenticated query received")
        
        logger.info(f"Processing query: {request.query[:50]}...")

        # Process query using SimpleRAGService (pass user context if available)
        user_context = current_user.username if current_user else "anonymous"
        response = await rag_service.answer_query(request.query, user_id=user_context)
