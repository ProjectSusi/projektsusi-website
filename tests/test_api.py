...
class TestQueryEndpoints:
    ...
    def test_query_endpoint_accessible(self, test_client):
        ...
        csrf_response = test_client.get("/api/v1/csrf-token")
        assert csrf_response.status_code == 200
        csrf_token = csrf_response.json()["csrf_token"]
        
        # Test with a simple query and CSRF token
        query_data = {"query": "What is the capital of France?"}
        headers = {"X-CSRF-Token": csrf_token}

        response = test_client.post("/api/v1/query", json=query_data, headers=headers)
    ...
    def test_query_endpoint_validation(self, test_client):
        ...
        response = test_client.post("/api/v1/query", json={}, headers=headers)
        assert response.status_code == 422  # Validation error

        # Test with empty query
        response = test_client.post("/api/v1/query", json={"query": ""}, headers=headers)
